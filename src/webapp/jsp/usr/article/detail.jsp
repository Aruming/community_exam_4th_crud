...
<section>
    <div class="container px-3 mx-auto">
        ...
        <div class="mt-3">
            ...
            <div class="mt-2">
                <a href="/usr/article/modify/${article.id}" class="btn btn-outline btn-info btn-sm">수정</a>
                <a onclick="if ( !confirm('정말로 삭제하시겠습니까?') ) return false;" href="/usr/article/delete/${article.id}?_method=DELETE" class="btn btn-outline btn-info btn-sm">삭제</a>
            </div>
            <div>
            ...
            <c:if test="${prevArticle != null}">
                <div class="mt-2">
                    <a href="/usr/article/detail/${prevArticle.id}" class="hover:text-[red]">
                        이전글 : <span class="badge badge-info badge-outline">${prevArticle.id}</span> ${prevArticle.title} - ${prevArticle.createdDate}
                    </a>
                </div>
            </c:if>
            <c:if test="${nextArticle != null}">
                <div class="mt-2">
                    <a href="/usr/article/detail/${nextArticle.id}" class="hover:text-[red]">
                        다음글 : <span class="badge badge-info badge-outline">${nextArticle.id}</span> ${nextArticle.title} - ${nextArticle.createdDate}
                    </a>
                </div>
            </c:if>
        </div>
            ...
            <c:forEach items="${articles}" var="listItemArticle">
                <li>
                    <a href="/usr/article/detail/${listItemArticle.id}" class="flex p-2 group">
                        <span class="badge badge-info badge-outline mr-2">${listItemArticle.id}</span>
